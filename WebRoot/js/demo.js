...
wx.ready(function () {
  ...
  // 4.4 监听录音自动停止 : 录音时间超过一分钟没有停止的时候会执行 complete 回调
  wx.onVoiceRecordEnd({
    complete: function (res) {
      voice.localId = res.localId;
      alert('录音时间已超过一分钟');
  ...
  function doUpload(serverId) {
	  $.ajax({
			url: "http://www.2510491430.com.tunnel.qydev.com/wechat-access/servlet/UploadImgServlet",
			type: "post",
			dataType: 'text',
...
//    });
    wx.openLocation({
    	...
    	name: '华宇(大连)信息服务有限公司',// 位置名
    	address: '大连市甘井子区金虹东路301号',// 地址详情说明
